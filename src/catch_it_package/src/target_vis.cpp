...
visualization_msgs::Marker createMarker(std::string frame_id, std::string ns, int id, double x, double y, double z, std::string colour)
{
...
int main(int argc, char **argv)
{
  ...
  while(ros::ok())
  {
    ...
    if (target.is_moving())
    {
      ...
      pos_marker = createMarker("/pos_marker_frame", "pos_markers", marker_id_counter,pos(0),pos(1),pos(2),"green");
      ...
      if (pred_target.is_moving())
      {
        ...
        pred_pos_marker = createMarker("/pos_marker_frame", "pred_markers", 0,pred_pos(0),pred_pos(1),pred_pos(2),"red");
